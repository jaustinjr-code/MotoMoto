...
{
    public class FetchTrendKpiRegistrationDataAccess : MariaDBConnectionBase, IFetchKpiDataAccess
    {
        private MySqlConnection? _mySqlConnection;
        private string _connectionString;

        public FetchTrendKpiRegistrationDataAccess()
        {
            // Use App.config here
            ...
            if (reader.HasRows)
            {
                ...
                try
                {
                    while (reader.Read())
                    {
                        ...
                        string y = reader.GetInt32("registrationTotal").ToString();
        ...
        public IUsageAnalyticEntity FetchChartMetrics(IUsageAnalyticModel analyticModel)
        {
            ...
            string commandText = "SELECT accessDate, registrationTotal FROM AdmissionAnalytics WHERE accessDate >= NOW() - INTERVAL 3 MONTH  AND accessDate < NOW();";
